...
export const initQueueService = async () => {
  ...
  if (preferredType === 'bullmq') {
    try {
      console.log('Attempting to initialize BullMQ + Redis...');
      initBullQueue();
      activeQueueType = 'bullmq';
      console.log('Using BullMQ as the active message queue.');
    } catch (err) {
      console.warn('BullMQ init failed. Falling back to Custom MongoDB-backed Queue.');
      activeQueueType = 'custom';
      await customQueue.recoverPending();
    }
  } else {
    console.log('Using Custom MongoDB-backed Queue.');
    activeQueueType = 'custom';
    await customQueue.recoverPending();
  }
};
...
export const enqueueMessage = async (messageId) => {
  if (activeQueueType === 'bullmq') {
    try {
      ...
      console.error('BullMQ enqueue failed. Falling back dynamically to Custom Queue.');
